...
int ReadBitmapFile (char* FileName,  BMPImageStruct *Image)
{
	...
	fread (&entier32,sizeof (4),1,ImageFile);
	Image->filesize = hex2dec(entier32, sizeof(4));	// taille totale du fichier, 4 octets
	fread (&entier32,sizeof(4),1,ImageFile);		// reserv 
	fread (&entier32,sizeof(4),1,ImageFile);		// offset de d but de l'image, 4 octets
	Image->offset = hex2dec(entier32, sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// taille de l'entete, 4 octets
	Image->headersize = hex2dec(entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// largeur de l'image, 4 octets
	Image->imageHsize = hex2dec(entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// hauteur de l'image, 4 octets
	Image->imageVsize = hex2dec(entier32,sizeof(4));
	...
	fread (&entier32,sizeof(4),1,ImageFile);		// compression (0=rien), 4 octets
	Image->compression = hex2dec(entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// taille de l'image, 4 octets
	Image->imagesize = hex2dec(entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// r solution horizontale en pixels par m tre, 4 octets
	Image->Hres = hex2dec (entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// r solution verticale, en pixels par m tre, 4 octets
	Image->Vres = hex2dec (entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// nombre de couleurs utilis es (0=toutes), 4 octets
	Image->colors = hex2dec (entier32,sizeof(4));
	fread (&entier32,sizeof(4),1,ImageFile);		// nombre de couleurs importantes (0=toutes), 4 octets
	Image->primarycolors = hex2dec (entier32,sizeof(4));
   
	// si c'est une image 8 bpp, cette ent te est suivie de la palette.
	if ((Image->palette = (unsigned char *) malloc(sizeof(unsigned char) * Image->colors * sizeof(4)))==NULL) return -1;
	for (int i = 0; i < Image->colors; i++) fread(&Image->palette[i*sizeof(4)],sizeof(4),1,ImageFile);
