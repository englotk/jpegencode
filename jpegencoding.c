...
void JPEGencoding(BMPImageStruct *InputImage , BMPImageStruct *OutputImage)
{
   ...
   for (int i=0; i<InputImage->imageVsize; i=i+8)
    ...
    for (int u=0; u<8; u++)
      {
       ...
       // 2me tape: Partie paire
       tmp10=tmp0+tmp3;
       tmp13=tmp0-tmp3;			// soit 4 ADD
       tmp11=tmp1+tmp2;
       tmp12=tmp1-tmp2;
       // 3me tape: Partie paire
    ...
    // On range le r sultat dans l'image de sortie
	 for (int u=0; u<8; u++)
	   for (int v=0; v<8; v++)
...
{
   int Block[8][8], row, col, row_mix, col_mix, packetoffset, buffersize;
   int packetsize=0;
   ...
   // On cr e et on initialise le codeur MQ
   objet=&mqobjet;
   ...
   while ((loop == true) && (BlockOffset != (unsigned int)(InputImage->imageHsize * InputImage->imageVsize /64))) {
	// On lit le bloc
	...
	// On cherche o  se trouve le dernier coef <> 0 selon le zig-zag
	K=63;
	...
	q=K / 2;	r=K % 2;
	for (unsigned int x=0; x<q; x++) mqc_encode(objet, 1);
	mqc_encode(objet, 0);
	mqc_encode(objet, r);
	...
	if ((unsigned int) buffersize < (MSS - 2)) {
		overhead = false;
   ...
   // On  crit le packet dans le fichier trace
...
//int compress(void)
{
	...
	char originalFilename[100] = "128x128-test.bmp";
	char targetFilename[126] = "";
	char newtargetFilename[126] = "";
	char shellCmd[276] = "";
